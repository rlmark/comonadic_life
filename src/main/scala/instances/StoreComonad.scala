...
object StoreComonad {
  ...
  implicit def storeComonadInstance: Comonad[CartesianStore] = {
    new Comonad[CartesianStore] {
      override def counit[A](w: CartesianStore[A]): A = w.query(w.index)

      override def coflatten[A](w: CartesianStore[A]): CartesianStore[CartesianStore[A]] = Store(Store(w.query))(w.index)
